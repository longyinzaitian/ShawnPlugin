...
public class HookPackageManager {
    ...
    public static void hookLoadedApkInActivityThread(File apkFile) throws ClassNotFoundException,
        ...
        Map mPackages = (Map) hookActivityThread.getPackages(activityThread);

        //准备两个参数
        // android.content.res.CompatibilityInfo
        HookCompatibilityInfo hookCompatibilityInfo = new HookCompatibilityInfo();
        Object defaultCompatibilityInfo = hookCompatibilityInfo.getDefaultCompatibilityInfo();
        ...
        Object loadedApk = RefInvoke.invokeInstanceMethod(activityThread, "getPackageInfoNoCheck", p1, v1);

        //为插件造一个新的ClassLoader
        String odexPath = FileUtil.getPluginOptDexDir(applicationInfo.packageName).getPath();
        String libDir = FileUtil.getPluginLibDir(applicationInfo.packageName).getPath();
